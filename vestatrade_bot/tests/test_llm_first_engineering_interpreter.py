...
def test_llm_interpreter_converts_well_rings_and_ambiguous_litres() -> None:
    llm = _EngineeringJSONLLM(
        {
            "handled": True,
            "continuation": True,
    ...
    assert "2,7 м" in response.answer and "1,8 м" in response.answer
    assert response.debug["slots"]["required_flow_m3_h"] == 6
    assert response.debug["slots"]["dynamic_water_level_m"] == 1.8
    assert response.debug["final_answer_source"] == "engineering_interpreter_llm"
...
def test_deterministic_safety_net_understands_ring_phrasing_without_llm() -> None:
    ...
    assert "~2,7 м" in response.answer
    assert "~1,8 м" in response.answer
