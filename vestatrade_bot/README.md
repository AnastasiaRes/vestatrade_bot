...
Там будет визуальный чат: слева переписка с ботом и карточки найденных товаров,
справа быстрые тестовые запросы, статус фида и debug-блок с распознанным
интентом, категорией и слотами. Административная перезагрузка фида намеренно
не вынесена в публичный интерфейс.
...
Проверка готовности каталога (для readiness probe):

```bash
curl http://127.0.0.1:8000/ready
```

`/health` проверяет, что процесс жив, а `/ready` возвращает `503`, пока каталог
не загружен.
...
Перезагрузка фида выключена, пока не задан `RELOAD_FEED_TOKEN`. После включения
endpoint всегда требует админский заголовок:
...
Для нескольких workers/контейнеров состояние диалога нужно вынести в Redis:
...
Без `SESSION_STORE_URL` используется локальное in-memory хранилище — это удобно
для разработки с одним worker, но не обеспечивает общий контекст между
процессами. Идентификаторы сессий в Redis хешируются, каждый turn защищён
распределённой блокировкой и откатывается целиком при ошибке.
...
Если модель вернула некорректный JSON, агент просит её повторить ответ. Все
LLM-агенты одной пользовательской реплики делят единый временной бюджет. Ollama
может использовать всё оставшееся время для текущей генерации; после исчерпания
бюджета новые LLM-вызовы не запускаются и бот продолжает детерминированно.
Быстрые сетевые ошибки можно повторить, но повтор не начинает новый бюджет.
...
`LLM_REQUEST_TIMEOUT_SECONDS=180` гарантирует общий предел в три минуты на
реплику, даже если оркестратор последовательно вызывает несколько агентов.
`LLM_TIMEOUT_SECONDS` остаётся отдельным пределом для вызовов вне диалогового
бюджета и для удалённых провайдеров.
...
Поддерживаются `.pdf`, `.txt`, `.md`. Документы подхватываются при старте и при
авторизованном `POST /reload-feed`; количество привязанных документов видно в `/health`
