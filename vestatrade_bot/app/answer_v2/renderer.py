...
_FACT_LABELS = {
    ...
    "static_water_level_m": "глубина до воды",
    "dynamic_water_level_m": "динамический уровень воды",
...
_LEARN_METHOD_INSTRUCTIONS = {
    "calculate_heat_loss_or_read_project_power": (
        "Укажите расчёт теплопотерь или проектную мощность отопления. Если их "
        "нет, назовите отапливаемую площадь: по ней можно показать только "
        "предварительные варианты. Для точного расчёта нужны ограждающие "
        "конструкции, климат, вентиляция и режим здания."
    ),
    ...
    "estimate_open_water_delivery_head": (
        "Укажите, на какую высоту нужно поднять воду от источника и примерно "
        "какова длина шланга или трубы до полива. Эти данные определяют "
        "нужный напор для открытой водяной линии."
    ),
    "calculate_borehole_required_head": (
        "Напор для скважины рассчитаю предварительно по уровню воды, высоте "
        "подъёма, трассе, трубе, расходу и нужному давлению. Рабочую точку "
        "насоса затем всё равно нужно сверить по кривой производителя."
    ),
    "confirm_borehole_required_flow": (
        "Укажите нужный расход воды — например, в л/мин или м³/ч. Это "
        "требование системы, а не максимальная подача насоса из карточки."
    ),
    "measure_dynamic_water_level": (
        "Нужна глубина уровня воды от поверхности; статический уровень можно "
        "использовать как менее точный предварительный ориентир."
    ),
    "measure_static_water_level": (
        "Укажите глубину статического уровня воды от поверхности земли в метрах."
    ),
    "measure_borehole_lift_height": (
        "Нужна высота от уровня воды до самой высокой точки разбора воды."
    ),
    "measure_borehole_horizontal_run": (
        "Нужна длина горизонтальной трубы от скважины до точки разбора воды."
    ),
    "confirm_borehole_required_pressure": (
        "Укажите нужное давление воды в доме в барах по манометру."
    ),
    "measure_borehole_discharge_diameter": (
        "Нужен внутренний диаметр напорной трубы; для ПНД можно указать наружный "
        "диаметр и SDR."
    ),
    "read_borehole_discharge_sdr": (
        "Прочитайте значение SDR на маркировке напорной ПНД-трубы возле диаметра."
    ),
    ...
    "identify_irrigation_water_source": (
