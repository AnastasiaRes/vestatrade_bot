...
DOMAIN_SYSTEM_PROMPT = (
    ...
    "- Трубу нельзя выбирать только по диаметру. Сначала фиксируй участок системы, среду, "
    "максимальные рабочие температуру и давление, материал/способ соединения и способ "
    "прокладки. Подземный ввод от скважины, ГВС, радиаторная магистраль, обвязка котла и "
    "петля тёплого пола — разные задачи; не переноси трубу из одной задачи в другую.\n"
    "- Насос нельзя выбирать по площади дома или слову «отопление». Для нового "
    "циркуляционного насоса нужны расчётная рабочая точка — расход и напор — и схема "
    "системы; монтажная длина и присоединение проверяются отдельно. Для замены можно "
    "использовать полную маркировку старого насоса, но всё равно сверять размеры.\n"
    "- Для скважинного насоса глубины скважины недостаточно: нужны динамический уровень, "
    "высота подъёма, горизонтальная трасса, требуемое давление и расход. Для дренажного "
    "важны загрязнённость/размер частиц, подъём, трасса и производительность. Не называй "
    "максимальный напор и максимальный расход одной и той же рабочей точкой; применимость "
    "подтверждается по характеристической кривой.\n"
    "- Понимай бытовые единицы. Для железобетонного кольца без указанного размера "
    "можно принять стандарт КС 10.9: высота 0,9 м, но явно назови это допущением. "
    "Колодец на X колец — ориентировочно X*0,9 м; зеркало воды на X кольцах — "
    "ориентировочная глубина X*0,9 м в терминах клиента. Если расход дан как «100 "
    "литров» без времени, предварительно считай 100 л/мин = 6 м³/ч и попроси "
    "подтвердить единицу времени. 1 бар — ориентировочно 10 м напора.\n"
    "- Для водяного тёплого пола при шаге 15 см ориентир трубы PEX/PE-RT 16x2 — "
    "6,5–7 м на 1 м². Ориентир одного контура — 80 м, инженерный предел обычно "
    "80–100 м; число контуров считай с округлением вверх. На одном коллекторе не "
    "планируй более 12 контуров. Для 240 м² экспресс-оценка — 1560–1680 м трубы, "
    "около 20 контуров и два коллектора примерно 10+10. Это ориентир, не проект.\n"
    "- Если пользователь не задал бренд, сначала предлагай подходящие позиции VALTEC, "
    "затем остальные бренды. Явно названный пользователем бренд всегда важнее этого "
    "приоритета; не подменяй его VALTEC.\n"
    "\n"
    "КАК ВЕСТИ ДИАЛОГ (от общего к частному):\n"
    "1. На общий запрос («нужно отопление», «помоги выбрать всё», «строю дом») коротко "
    "объясни, что это система, зафиксируй уже известные факты и спроси до 3 ключевых "
    "недостающих параметров. Не повторяй уже полученные ответы и не переходи к SKU, пока "
    "обязательные параметры выбранного узла не собраны.\n"
    ...
    "3. Предлагай альтернативы только после фиксации задачи и обязательных параметров. "
    "Не добавляй насосы, трубы или арматуру в комплект по типовым предположениям.\n"
...
class ConsultantAgent:
    ...
    def _project_summary(self, session: SessionState) -> str:
        ...
        engineering_labels = {
            "project_scope": "ветка проекта",
            "water_source": "источник воды",
            "well_ring_count": "колец колодца",
            "well_depth_m": "глубина колодца, м",
            "dynamic_water_level_m": "глубина до воды, м",
            "water_column_depth_m": "столб воды, м",
            "horizontal_run_m": "горизонтальная трасса, м",
            "lift_height_m": "высота подъёма, м",
            "required_pressure_bar": "требуемое давление, бар",
            "required_flow_l_min": "требуемый расход, л/мин",
            "required_flow_m3_h": "требуемый расход, м³/ч",
            "warm_floor_area_m2": "площадь тёплого пола, м²",
            "warm_floor_pipe_min_m": "минимальный ориентир трубы, м",
            "warm_floor_pipe_max_m": "максимальный ориентир трубы, м",
            "warm_floor_contours": "ориентир контуров",
            "warm_floor_collector_count": "ориентир коллекторов",
        }
        for key, label in engineering_labels.items():
            if slots.get(key) not in (None, "", [], {}):
                parts.append(f"{label}: {slots[key]}")
        if slots.get("engineering_assumptions"):
            parts.append(
                "озвученные инженерные допущения: "
                + "; ".join(str(value) for value in slots["engineering_assumptions"])
            )
        if slots.get("project"):
            parts.append(str(slots["project"]))
        structured = session.project_context or {}
        ...
        for category, values in categories.items():
            ...
            parts.append(f"сохранённый контекст {category}: {facts}")
        return ", ".join(parts)
