...
TOPICS: tuple[CommerceTopic, ...] = (
    ...
    CommerceTopic(
        ...
        # «Резервный котёл», «каскад для резерва», «резервирование мощности» —
        # инженерное резервирование оборудования: живой прогон 25.08 увёл по
        # этому слову проектный запрос на каскад 240 кВт в регламент брони
        # товара на складе (B12).
        excludes=(
            "резервуар",
            "резервн",
            "для резерва",
    ...
    CommerceTopic(
        ...
        label="монтаж и услуги",
        # Живой прогон 25.08 (A20): «А монтаж вы делаете?» уходило в подбор
        # трубы, и прямого «да» или «нет» покупатель не получал за восемь
        # ходов. Услуги монтажа в проверенных фактах нет — так и говорим, а
        # не молчим и не обещаем.
        markers=(
            "монтаж",
            "установку сделаете",
            "устанавливаете",
        ...
        # «Монтажная длина» и «монтажный размер» — параметр насоса, а не услуга.
        excludes=(
            "монтажн",
            "монтажу трубы",
        ),
        ...
        note=(
            "Подтверждённой услуги монтажа в моих данных нет, поэтому работы "
            "от лица компании я не обещаю. Оборудование и всю обвязку подберу "
            "здесь, а делает ли компания монтаж и по какой цене — подтвердит "
            "менеджер."
        ),
    ),
    CommerceTopic(
    ...
    CommerceTopic(
        ...
        # «Скиньте» — омоним: сбросить цену и прислать файл. Живой прогон
        # 25.08 (D20): «Скиньте схему подключения бойлера, картинкой» трижды
        # получил регламент по скидкам. Здесь перечислено то, что «скидывают»
        # в смысле «пришлите».
        excludes=(
            "скиньте схем",
            "скиньте ссылк",
...
def _marker_is_only_negated(text: str, marker: str) -> bool:
    """«Я не про скидку» — упоминание темы, а не обращение по ней.

    Живой прогон 25.08 (D20): покупатель третий раз пишет «Нет, я не про
    скидку, мне нужна схема обвязки» — и получает регламент по скидкам,
    потому что слово «скидк» в реплике есть. Тема отбрасывается только
    тогда, когда ВСЕ её упоминания отрицательные: «скидка не нужна, но
    посчитайте объём» темой остаётся.
    """

    found = False
    for match in re.finditer(re.escape(marker), text):
        found = True
        prefix = text[max(0, match.start() - 26) : match.start()]
        if not _TOPIC_NEGATION_RE.search(prefix):
            return False
    return found
    ...
    for topic in TOPICS:
        ...
        for marker in topic.markers:
            if marker not in text:
                continue
            if _marker_is_only_negated(text, marker):
                continue
...
def describe_reachable_phones(facts: BusinessFacts, *, limit: int = 6) -> str:
    """Телефоны, по которым отвечает живой сотрудник, по одному на город.

    Прямой линии конкретного менеджера в конфигурации нет, и выдумывать её
    нельзя.  Но телефоны точек проверены, и это ровно те номера, по которым
    покупатель дозвонится до человека.  В прогоне 25.08 отказ без единого
    номера (A12, A21, B21, C05, D10, D11) заканчивал разговор ничем — при том
    что проверенные телефоны всё это время лежали в конфигурации.
    """

    seen: dict[str, str] = {}
    for branch in facts.branches:
        if branch.phones and branch.city not in seen:
            seen[branch.city] = branch.phones[0]
    if not seen:
        return ""
    lines = [f"- {city}: {phone}" for city, phone in list(seen.items())[:limit]]
    return "\n".join(lines)
...
def compose_store_contact_answer(
    ...
    *,
    ...
    if "personal" in requested:
        # Личные номера и мессенджеры сотрудников не раскрываем — но отказ без
        # альтернативы обрывает разговор, поэтому сразу даём рабочий канал.
        answer = (
            "Личные телефоны и мессенджеры сотрудников я не даю — это их "
            "персональные данные. Рабочие телефоны наших точек открыты, по ним "
            "отвечает живой сотрудник:"
        )
        phones = describe_reachable_phones(facts, limit=3)
        if phones:
            return (
                f"{answer}\n{phones}\n"
                "Если вопрос срочный, оставьте телефон или email — подготовлю "
                "обращение и покажу его состав перед отправкой."
            )
        if facts.site_url:
            return (
                "Личные телефоны и мессенджеры сотрудников я не даю — это их "
                "персональные данные. Официальные контакты компании — на "
                f"{facts.site_url}."
            )
        return (
            "Личные телефоны и мессенджеры сотрудников я не даю — это их "
            "персональные данные. Могу подготовить обращение менеджеру: "
            "оставьте телефон или email."
        )
    if "manager" in requested:
        # Живого сотрудника покупатель получает по телефону точки. Отдельной
        # прямой линии менеджера в конфигурации нет — так и говорим, но номер
        # даём, иначе просьба «дайте человека» упирается в отказ.
        if city:
            branches = facts.branches_in(city)
            if branches:
                answer = (
                    "Прямого номера конкретного менеджера у меня нет, но по "
                    f"телефонам точки в городе {city} отвечает живой сотрудник:\n"
                    f"{describe_branches(branches)}"
                )
                if facts.emails:
                    answer += "\nОбщая почта: " + ", ".join(facts.emails) + "."
                return (
                    answer
                    + "\nЕсли удобнее, чтобы позвонили вам, — оставьте телефон "
                    "или email, я подготовлю обращение и покажу его состав "
                    "перед отправкой."
                )
        # Честная часть остаётся дословно: прямой линии менеджера нет, и склад
        # вне каталога боту недоступен. К ней добавляется то, чего в живом
        # прогоне не хватало, — рабочий телефон, по которому ответит человек.
        base = (
            ...
            "индивидуальную цену не могу."
        )
        phones = describe_reachable_phones(facts)
        if phones:
            return (
                f"{base} Но по телефонам наших точек отвечает живой сотрудник:\n"
                f"{phones}\n"
                "Если удобнее, чтобы позвонили вам, оставьте телефон или email — "
                "подготовлю обращение и покажу его состав перед отправкой."
            )
        return (
            f"{base} Могу подготовить обращение менеджеру: для обратной связи "
            "понадобится ваш телефон или email, затем я покажу состав и "
            "отдельно попрошу согласие на передачу."
        )
    ...
    # Спросили телефон — надо дать телефон. Встречный вопрос «назовите город»
    # уместен для адреса и режима работы, но на просьбу «дайте номер» он
    # превращается в круг: в прогоне 25.08 покупатель города не называл и
    # уходил (A21, C07, D18, C24).
    if requested.intersection({"phone", "manager"}) and not requested.intersection(
        {"address", "url"}
    ):
        phones = describe_reachable_phones(facts)
        if phones:
            return (
                "Телефоны наших точек — по любому из них ответит сотрудник:\n"
                f"{phones}\n"
                "Назовите город, если нужен адрес и режим работы конкретной точки."
            )
