...
# «мне» здесь раньше было обязательным — так отличали просьбу «дай мне
# телефон» от передачи своего («я дам телефон»).  Отличать по слову больше не
# нужно: реплику с настоящим номером внутри снимает проверка владения
# контактом в ``frame``.  А требование «мне» стоило живому прогону 25.08
# ответа на голое «Телефон дайте» (D11) — оно уходило в out_of_scope.
_STORE_CONTACT_IMPERATIVE_RE = re.compile(
    r"\b(?:дай\w*|пришл\w*|отправ\w*|напиш\w*|подскаж\w*|скаж\w*|сообщ\w*)\b"
    r"[^.!?]{0,36}"
    r"\b(?:телефон\w*|номер\w*|email|e-mail|имейл\w*|почт\w*|"
    r"контакт\w*|кантакт\w*)\b"
    r"|\b(?:телефон\w*|номер\w*|email|e-mail|имейл\w*|почт\w*|"
    r"контакт\w*|кантакт\w*)\b[^.!?]{0,20}"
    r"\b(?:дай\w*|пришл\w*|отправ\w*|скаж\w*|сообщ\w*)\b"
)
...
def _requested_store_contact_channels(text: str) -> tuple[str, ...]:
    channels: list[str] = []
    # Личный контакт конкретного сотрудника — отдельный запрос: официальные
    # рабочие телефоны на него не отвечают, нужен явный отказ (C07).
    if re.search(
        r"\b(?:личн\w*|мобильн\w*|сотов\w*|whatsapp|ватсап\w*|вотсап\w*|"
        r"телеграм\w*|telegram)\b[^.!?]{0,28}"
        r"\b(?:директор|менеджер|сотрудник|владелец|начальник|продавец)\w*\b"
        r"|\b(?:директор|менеджер|сотрудник|владелец|начальник|продавец)\w*\b"
        r"[^.!?]{0,28}\b(?:личн\w*|мобильн\w*|сотов\w*|whatsapp|ватсап\w*|"
        r"вотсап\w*|телеграм\w*|telegram)\b",
        text,
    ):
...
class TurnPlanner:
    ...
    def frame(
        ...
        *,
        ...
        # Живой прогон 25.08 (A13, D17, D18, D10): «Напиши, что я оставляю
        # телефон +7 999 …» разбиралось как просьбу дать контакт магазина —
        # совпали существительное «телефон» и повелительное «напиши».  Контакт
        # покупателя после этого гасился (`customer_contact_present` ниже
        # умножается на `not store_contact`), заявка не создавалась, и в ответ
        # уходил список пунктов выдачи.
        #
        # Названный в реплике номер или адрес — признак сильнее любой
        # формулировки: это акт передачи контакта, а не запроса.  Исключения
        # оставлены там, где покупатель явно адресует вопрос магазину («ваш
        # телефон», «адрес в Самаре»): тогда в одной реплике действительно два
        # действия и приоритет справочного ответа сохраняется.
        if (
            owned_customer_contact
            and store_contact
            and not _STORE_CONTACT_POSSESSIVE_RE.search(text)
            and not _STORE_CONTACT_LOCATION_RE.search(text)
            and not _STORE_CONTACT_GENERIC_QUESTION_RE.search(text)
        ):
            store_contact = False
