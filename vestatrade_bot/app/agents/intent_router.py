...
# «Старого насоса нет» содержит и «стар», и «насос», поэтому проверка на
# слова-признаки замены срабатывала на прямом отрицании и уводила покупателя,
# который просит новый подбор, в сценарий замены — с требованием прислать
# маркировку насоса, которого у него нет.
_NO_OLD_PUMP_RE = re.compile(
    r"стар\w*\s+насос\w*[^.!?]{0,24}\bнет\b"
    r"|\bнет\b[^.!?]{0,16}стар\w*\s+насос"
    r"|\bбез\s+стар\w*\s+насос"
    r"|стар\w*\s+насос\w*\s+не\s+был",
    re.IGNORECASE,
)


def _denies_an_old_pump(text: str) -> bool:
    """Покупатель прямо сказал, что старого насоса нет."""

    return bool(_NO_OLD_PUMP_RE.search(text))
...
# Обороты, вводящие место установки, а не предмет запроса. Покупатель
# описывает узел десятком способов — «перед насосом», «после насосной
# станции», «на входе в котёл», «между насосом и баком», «на обвязке котла», —
# и во всех случаях названный так узел остаётся ориентиром монтажа.
_LANDMARK_RE = re.compile(
    r"\b(?:перед|после|до|за|возле|около|рядом\s+с|вместо|между|"
    r"на\s+входе\s+в|на\s+выходе\s+из|на\s+вводе\s+в|"
    r"на\s+подаче\s+(?:к|в)|на\s+обратке\s+(?:к|от)|на\s+линии\s+(?:к|от)|"
    r"на\s+подводке\s+к|на\s+обвязке|на\s+всасывании\s+у?|"
    r"со\s+стороны|перед\s+входом\s+в|сразу\s+за)\b"
    r"(?:\s+\S+){1,4}"
)
...
class IntentRouterAgent:
    ...
    def _detect_category(self, text: str) -> tuple[str, float]:
        ...
        #
        # Головным слово считается только вне оборота места. «Фильтр перед
        # насосом» и «кран после насосной станции» называют насос как ориентир
        # монтажа, а покупателю нужен фильтр и кран; безусловная проверка
        # уводила такие запросы в насосы. «Насос для канализации» оборотом
        # места не является, и там насос остаётся предметом запроса.
        head_text = _strip_landmark_phrases(text)
        if re.search(r"\b(?:насос|помп)\w*\b", head_text):
            return "pumps", 0.99
        if re.search(r"\b(?:кран\w*\s+маевск\w*|ручн\w*\s+воздухоотвод\w*)\b", text):
            return "radiator_fittings", 0.99
        if re.search(r"\bзапорн\w*\s+арматур\w*\b", head_text):
            return "valves", 0.99
    ...
    def _extract_slots(
        ...
        if (
            ...
            slots["pump_use"] = "отопление"
            slots["pump_context"] = "котел"
        if category == "pumps" and not _denies_an_old_pump(text) and (
            "на замен" in text
