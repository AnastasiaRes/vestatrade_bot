...
class HandoffAgent:
    # Служебная сводка для менеджера, показанная покупателю, длиннее этого
    # предела перестаёт быть сводкой и становится стеной текста.
    _PREVIEW_LIMIT = 220

    @staticmethod
    def _reachable_hint() -> str:
        """Как позвонить прямо сейчас, по проверенным телефонам точек.

        Импорт локальный: конфигурация фактов не должна тянуться в модуль
        передачи при загрузке, а список телефонов нужен только здесь.
        """

        try:
            from app.business_config import get_business_facts

            from .commerce_topics import describe_reachable_phones

            phones = describe_reachable_phones(get_business_facts(), limit=3)
        except Exception:  # конфигурация фактов не должна ломать передачу
            phones = ""
        if phones:
            return "по телефонам наших точек:\n" + phones
        return "по контактам с официального сайта."
    ...
    def compose_consent_request(
        ...
        *,
        ...
        # Живой прогон 25.08: preview склеивал все реплики покупателя подряд и
        # разрастался до пяти строк дословного пересказа (A09, B20, C06, C24).
        # Покупателю он ничего не сообщает — это служебная сводка, — а формат
        # ответа ломает.
        if len(preview) > self._PREVIEW_LIMIT:
            preview = preview[: self._PREVIEW_LIMIT].rstrip(" ,;") + "…"
        if needs_contact:
            # Единственный предложенный путь «оставьте контакт» оказался
            # тупиком: покупатель контакт не давал и уходил. Второй путь —
            # позвонить самому — доступен сразу и не требует ничего от него.
            return (
                "Заявку менеджеру пока не отправляю. Подготовил данные для "
                f"передачи: {preview}.\n"
                "Дальше на выбор: оставьте телефон или email — покажу итог и "
                "попрошу подтвердить передачу; либо позвоните сами, "
                f"{self._reachable_hint()}"
            )
        return (
