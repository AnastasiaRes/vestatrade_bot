...
MANAGER_PERSONA = (
    ...
    " Загрузка фотографий и изображений в этом чате не поддерживается. Никогда не проси "
    "покупателя прислать, отправить, загрузить или прикрепить фото сюда; вежливо сообщи "
    "об ограничении и предложи переписать маркировку и описать видимые признаки словами."
)
...
class ResponseComposerAgent:
    ...
    TERM_GLOSSARY: tuple[tuple[tuple[str, ...], str], ...] = (
        ...
        (
            ("теплопотер",),
            "Теплопотери здания — количество тепла, которое дом теряет через стены, "
            "окна, крышу, пол и вентиляцию при расчётной уличной температуре; их обычно "
            "выражают в ваттах или киловаттах. Точный расчёт делают по помещениям с "
            "учётом города/климата, размеров и материалов ограждений, утепления, окон, "
            "высоты потолков, вентиляции и желаемой температуры. Владельцу достаточно "
            "собрать эти исходные данные и передать проектировщику или специалисту по "
            "отоплению. Оценка только по площади годится лишь как предварительный ориентир, "
            "а не как окончательная мощность котла.",
        ),
        (
            ("котел", "котёл"),
