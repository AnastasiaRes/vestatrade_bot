...
_SKU_TOKEN_RE = re.compile(
    r"(?<![a-zа-я0-9])"
    r"(?:[a-zа-я]{1,8}(?:[.\-]|\s+)\d+(?:[.\-][a-zа-я0-9]+){0,5})"
    r"(?![a-zа-я0-9])",
    re.IGNORECASE,
)
_NUMERIC_SKU_RE = re.compile(r"\b\d{6,}\b")
...
class ProductFactEvidenceService:
    ...
    def resolve_reference(
        ...
        sku_tokens = [match.group(0).strip() for match in _SKU_TOKEN_RE.finditer(question)]
        if any(marker in text for marker in ("артикул", "sku", "код товара")):
            sku_tokens.extend(_NUMERIC_SKU_RE.findall(question))
        resolved = []
        ambiguous = []
    ...
    def _looks_like_unmapped_product_fact(
        ...
        has_product_reference = bool(
            ...
            or product_ref.raw
            or _SKU_TOKEN_RE.search(question)
            or _NUMERIC_SKU_RE.search(question)
        )
...
def render_product_fact_evidence(evidence: ProductFactEvidence) -> str:
    ...
    if evidence.status == ProductFactStatus.ANSWERED and evidence.value is not None:
        label = spec.label if spec else predicate.replace("_", " ")
